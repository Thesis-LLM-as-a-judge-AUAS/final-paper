...
%Rather than relying on labeled data or manual prompt design, ZEPO formulates a zero-shot fairness objective based on the ideal uniform distribution of LLM preferences.
...
%Experimental results show that ZEPO achieves 10–17% relative gains in Spearman correlation with human judgment over strong pairwise baselines.
%When combined with position debiasing (e.g., Balanced Position Calibration), ZEPO further improves performance, achieving up to 32% gains in specific categories such as dialogue engagement.
...
%\hline
%ZEPO & Position & Zero-shot prompt optimization for fairness-based preference calibration & Basic optimizer limits performance gains \\
\hline
